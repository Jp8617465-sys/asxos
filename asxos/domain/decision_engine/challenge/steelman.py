...
_RECOMMENDATION_VERBS: Final = re.compile(
    ...
    r"take profit|price target|target price|top pick|strong (?:buy|sell)|recommend(?:ed|s|ation)?)\b",
    re.IGNORECASE,
)
...
def admit_llm_finding(
    *,
    ...
    """The single door for LLM-authored findings. Refuses what D12/D14 forbid."""
    if not evidence_ids:
        raise BoundaryError("an LLM finding must cite at least one evidence id")
    for candidate in (text, required_response):
    ...
    return ChallengeFinding(severity=severity, finding=text, required_response=required_response, evidence_ids=evidence_ids[:100])
