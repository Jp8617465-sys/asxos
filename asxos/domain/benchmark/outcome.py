...
class LotInput:
    ...
    ``close_native`` is `prices.close` in the symbol's own currency; ``fx_audusd`` is
    USD per 1 AUD and is required only for a foreign symbol.
    """
    ...
    close_native: Decimal | None
    fx_audusd: Decimal | None
...
def _market_value_aud(lot: LotInput) -> tuple[Decimal | None, ReturnState, str]:
    ...
    if is_foreign_symbol(lot.symbol):
        if lot.fx_audusd is None or lot.fx_audusd <= 0:
            return (
                None,
                ReturnState.unavailable_no_fx,
                f"no AUDUSD rate — {lot.symbol} is priced in USD and cannot be stated in AUD",
            )
        value = lot.quantity * lot.close_native / lot.fx_audusd
    else:
        value = lot.quantity * lot.close_native
    return value.quantize(_SCALE), ReturnState.measured, ""


def _lot_return(lot: LotInput) -> tuple[Decimal | None, Decimal | None, ReturnState, str]:
    """(market_value_aud, return_fraction, state, note) — both legs in AUD."""
    value, state, note = _market_value_aud(lot)
    if value is None:
...
def _benchmark_measurement(
    ...
    if start.level <= 0:
        return (
            None,
            BenchmarkState.unavailable_no_series,
            f"benchmark level {start.level} at {start.as_of} is not positive",
            None,
        )
    return (
...
def build_lot_outcome(lot: LotInput, as_of: date) -> LotOutcome:
    ...
    value, lot_return, return_state, return_note = _lot_return(lot)
