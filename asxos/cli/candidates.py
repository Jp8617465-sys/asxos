...
def candidates_build(
    ...
    asyncio.run(_build(theme, symbol, date.fromisoformat(as_of), twice, persist))


async def _build(theme_code: str, symbol: str, as_of: date, twice: bool, persist: bool) -> None:
    await init_pool()
    try:
        async with acquire() as conn:
            tv = await build_theme_version(conn, theme_code=theme_code, as_of=as_of)
            cand = await build_candidate_snapshot(conn, symbol=symbol, theme=tv, as_of=as_of)
            if twice:
                tv2 = await build_theme_version(conn, theme_code=theme_code, as_of=as_of)
                cand2 = await build_candidate_snapshot(conn, symbol=symbol, theme=tv2, as_of=as_of)
