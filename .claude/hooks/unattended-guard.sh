...
case "$tool" in
  Bash)
    ...
    # A1 — push/merge/deploy/CI to main (Tier 6). Loose '\bgit\b … \bpush\b' matching so
  ...
  *execute_sql)
  ...
  mcp__github__get_*|mcp__github__list_*|mcp__github__search_*|mcp__github__pull_request_read|mcp__github__issue_read|mcp__github__actions_get|mcp__github__actions_list|mcp__github__get_me)
    ...
    ;;
  mcp__github__*)
    deny "unattended-guard: GitHub write (files/PR/branch/CI/release) is Tier 5/6, reserved to James. Draft via a branch + PR the interactive session opens."
    ;;
  ...
  mcp__supabase__*|mcp__Supabase__*)
    deny "unattended-guard: Supabase write/deploy/branch mutation is Tier 5, reserved to James. Reads go through mcp__supabase-ro__*."
    ;;
