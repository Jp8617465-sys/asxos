...
| Autonomy authority | `../AGENTS.md` is the cross-harness contract; `product/autonomy-policy.md` is its enforcement map and fail-closed activation checklist. `AGENTS.md` §0 remains the ceiling until the control ledger attests the exact policy digest, authoritative verifier commit, Ledger Writer identity and append-only protected Git history, and `AUTONOMY=STANDING`. |
