...
| Product vision / program state | `product/north-star.md` (the charter — "The Output") + `product/roadmap-state.md` (living reconciled roadmap + arbi's decision-log memory) + `product/arbi-harness.md` (arbi's operating contract + permission tiers) + `product/arbi-evals.md` (how arbi is measured). `/arbi` ("wake up") is the session entry ritual; `/arbi-close` writes the handoff. |
