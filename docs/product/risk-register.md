...
| R9 | **Brief hard-couples to model approval** — V1 `collect()` (`compose.py:190`) + V2 `active_theses` collector call `resolve_production_model()` unconditionally (RuntimeError on 0 approved); revoking approval to harden the quarantine would hard-fail the whole brief / hide every thesis card — the Model-A-*independent* discipline layer | P1 | open | Make the Model A driver line best-effort (skip on 0 approved) — owner `backend-architect`. Until fixed, never revoke approval as a quarantine mechanism | scan `wf_f54323f5-d7d`; `active_theses.py:72-92` |
