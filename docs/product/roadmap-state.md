...
**§2.4 entry-gate status after this amendment** — one item remains open:
...
| (2) merged fixes have their required production or scheduled-run observations recorded | ❌ **OPEN** — the scheduled ~20:30 UTC `sync_prices` has not run. Owner: next session's observation. Evidence: a `job_runs` SUCCESS row for the scheduled run |
...
| 2 | **`derive_fundamentals_pit` failed 4 consecutive weekly runs** (`TimeoutError`; last success 2026-07-11) | ✅ **FIXED + OBSERVED GREEN (2026-08-12).** PR #85's fix executed its first production run under James's authority: 68 batches, 3,359 source symbols, **53,624 rows / 3,357 symbols in 148s** (`job_runs` success; was 63 rows / 11 symbols), zero timeouts. Coverage reconciled: 1,853 of 2,391 active-universe symbols covered — the 538 uncovered actives have no `rs_financial_statements` source rows at all (upstream sync coverage, a separate item, not a PIT defect). `compute_factor_scores` then consumed the cross-section: **3,308 symbols scored** at `as_of` 2026-08-11 (`fs_v1`; was 11). `check_cron_health` observed green the same session (first success in 12+ days) after the week-long consecutive-failure red cleared |
| 3 | **`prices` destructively upserted** (`asxos/ingestion/prices.py:53-59`) — every dividend/split silently rewrote `adj_close` history. The only defect where delay causes permanent loss | 🟡 **APPLIED IN PRODUCTION 2026-08-12; close-out pending.** PR #84 (`d0dbee0`) authored the migration, PR #88 (`97cdc5c`) production-readied it, and it is now **applied as `20260812092925` — observed count 96** (James authorised, attended session; pre-apply gate: drill run `31574011421` green on `97cdc5c`). All three triggers live; runbook probe captured exactly one same-transaction revision and rolled back, residue 0. `REQUIRED_MIGRATIONS` 95→96 **merged 2026-08-12 via PR #91** (`ec30d20`). The post-apply `restore_drill` is **DONE and green** — run `31593927269` logged `price_revisions exists — include append-only price history` and `all 14 table counts match`. **Sole remaining evidence: a `job_runs` SUCCESS row for the SCHEDULED ~20:30 UTC `sync_prices`.** Verified 2026-08-12 13:40Z: not yet present — the only `sync_prices` row for today is a `failure` at 12:45Z from a blocked *local* attempt (this machine's Python rejects an intercepting TLS certificate; `curl` succeeds, so it is an environment fault, not a code fault). Do not infer success from elapsed time and do not substitute a local run. Zero revision growth is an acceptable outcome if the provider returns byte-equivalent rows; what must be observed is that ingestion **succeeds** against the live capture trigger. Prospective only — **no historical backfill**, the pre-0043 `adj_close` rewrites are already lost |
