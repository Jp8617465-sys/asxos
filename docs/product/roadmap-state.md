...
| 3 | **`prices` destructively upserted** (`asxos/ingestion/prices.py:53-59`) — every dividend/split silently rewrote `adj_close` history. The only defect where delay causes permanent loss | ⚠️ **CONTAINED IN CODE, MIGRATION UNAPPLIED.** PR #84 (`d0dbee0`) adds `migrations/0043_price_revisions.sql`: append-only revision ledger, DELETE tombstones, UPDATE/DELETE/TRUNCATE blocked on the ledger, TRUNCATE blocked on `prices`, SECURITY DEFINER trigger with fixed `search_path`. **DB shows 95 applied migrations, latest `20260724110030` (0041) — 0043 is NOT applied, so history is still being overwritten today.** Rehearsal gate: **MET 2026-08-11** — PR #90 ran 0043 verbatim against a disposable PostgreSQL 17 (schema from `0001_initial.sql`), run `31481113459` observed all 10 scenarios green incl. same-xid atomic capture, the defect-#3 upsert path recoverable, ledger immutability (SQLSTATE 55000 ×4), SECURITY DEFINER capture from a zero-grant role, and fail-closed on an unledgered column (SQLSTATE 23514); evidence comment on #90. Remaining (James): apply 0043 via the Supabase workflow, bump `REQUIRED_MIGRATIONS` 95→96. Prospective only — **no historical backfill**, the pre-0043 `adj_close` rewrites are already lost |
