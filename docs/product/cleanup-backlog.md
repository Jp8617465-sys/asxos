...
| R4 | Label `compute_opportunity_cost`'s signal-driven ranking non-authoritative under rule #11 (now **standing policy** — the dispute is resolved; suppress the Model A read since the engine is shelved) — the midpoint fallback already exists (`:60-66`) | [REV] (code+review) | backend-architect |
...
| ~~R6~~ | ~~Consolidate the dual regime source onto the independent `market_context_current.regime_label` — V1 reads Model A `signals.regime` (`compose.py:204`), a display-only Model A leak~~ — **CLOSED by P1-04 (2026-08-13, PR #103), by deletion rather than the consolidation this item proposed:** V1's `signals.regime` read is gone — the brief's Model A display reads were removed outright, and `asxos/brief/compose.py` now issues no `signals` query in any gate state (`tests/test_brief_compose.py`). No consolidation onto `market_context_current.regime_label` occurred or is pending; the brief simply carries no regime line from `signals`. See `docs/product/model-a-reference-manifest.md`, P1-05 Tier-1 disposition ("R6 is closed (V1's `signals.regime` read is gone)") | [REV] closed | arbi (verify) |
...
| RC3 | **Repair `market_context_current` feed gaps** — **RBA + VIX FIXED (2026-07-11, needs live confirm):** `AUCBCNTO`→`IRSTCI01AUM156N` (OECD MEI immediate/call rate, the standard FRED RBA-cash-rate proxy) and `VIX.US`→`VIX.INDX` (.INDX convention, cf. AXVI.INDX) in `ingest_market_context.py`. Researched, **not live-verified** (no FRED/EODHD key + proxy blocks both hosts in the authoring env) — confirm the first values on Render's next `ingest_market_context` run. **Iron ore left unguessed:** `IRON.COMM` likely invalid but EODHD may not carry it; probe `TIO.COMM`/`FEF.COMM` with a live key or leave `iron_ore_62fe` soft-degrading to null. All three are soft-degrade (not hard-fail) | [REV] partial — RBA/VIX done, iron-ore + live-verify pending | backend-architect |
