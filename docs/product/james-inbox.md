...
| 🔵 **OPEN 2026-07-27 — ratify the personal-advice firewall amendment (advise + stage orders)** | Policy + boundary change to a non-negotiable. Per `arbi-constitution.md` §reserved authorities, arbi may only **draft** this — never self-enact. James ratifies by merging the authority-file PR the proposal specifies. | **Nothing to build — this is a read-and-merge decision.** `docs/proposals/personal-advice-firewall-amendment-2026-07-18.md` (status: proposed DRAFT) already scopes exactly the grant James chose in the 2026-07-27 Codex session (`1A`: direct personal advice + full order staging; sizes from a **model-independent sizer**, never Model A; James places every order — autonomous execution explicitly declined). It gates the sizer + staged-order lanes downstream (§4a names the sizer as its build dependency). **Independent of the pending Codex dossier** — ratifying before ~08-01 means the dossier lands against an unblocked repo. Does not touch rule #11. |
