...
|---|---|---|
| **HUBS `conviction_level`** — NULL on the thesis (and on all 13 theses, risk R11) | Policy/conviction: a conviction number is a governor value judgement, not something arbi can infer from data. Load-bearing here because HUBS is ~100% of capital. | James's **1–5** number for HUBS. Then the size-vs-conviction check (`portfolio-coherence-reviewer`) can finally run on the one name that is all the capital. Records to `theses.conviction_level` via CLI. |
