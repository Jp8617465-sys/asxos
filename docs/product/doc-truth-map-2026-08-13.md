...
| 2 | **Fresh verdict for dark-launch surface #2** (news/sentiment brief) | It is now formally UN-SHIPPED with no verdict. arbi can propose; the open question is whether `ASXOS_NEWS_BRIEF_ENABLED` goes to `0` in `daily-brief.yml:61` pending the symbol-mapping fix, or stays `1`. Not obviously firewall-crossing, so arbi *may* own it — but it has sat unowned for 8 days, which is why it needs naming |
