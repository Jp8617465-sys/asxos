...
All four must hold, checked in this order. The first that fails ends the fire with
`outcome=noop reason=<letter>`; no handoff file, no PR, no roadmap edit.
...
- **(d) A ready item exists**, first match wins:
  ...
  2. `python scripts/backlog_next.py` exits 0 — its pick is the item (exit 3 = nothing
     eligible; exit 2 = schema error → that is the item);
