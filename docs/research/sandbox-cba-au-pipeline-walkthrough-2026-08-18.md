...
## 4. Model A — dead residue, not a dormant system (correction)

**Correction, made after initial publication of this doc.** The section below originally described Model A as "quarantined... dormant," which understated what's actually true and was pointed out directly. The accurate state, verified against this branch's own git history:

`CLAUDE.md` rule #11 and `north-star.md` describe Model A as policy-quarantined (standing since 2026-07-11: 19,032 matured `signal_outcomes`, `corr(ml_prob, 21d return) = −0.03`, STRONG_BUY underperforming HOLD at 21 days). What I missed is that the *runtime itself has since been physically removed from the codebase* — **PR #100, "P1-02: remove Model A runtime, API, CLI and job dependencies," merged 2026-08-13, and is already an ancestor of this branch.** I confirmed directly: `asxos/domain/models/model_a.py`, `cache.py`, `asxos/domain/signals/writer.py`, `jobs/generate_signals.py`, `check_model_staleness.py`, and `track_signal_outcomes.py` are all gone from the working tree. Nothing in this system computes or writes a Model A signal anymore.

That PR made **no DB changes** ("no DB statements, no migrations" per its own commit message) — the `signals` table wasn't dropped, so the rows below are still queryable. But they are **orphaned historical residue**, not a live-but-untrusted feed: the last `generate_signals` run in `job_runs` is 2026-08-05, eight days before the code that produced it was deleted, and nothing will ever add a new row again. I pulled them without checking whether the producing code still existed — that was the actual error, not the decision to cite rule #11.
...
For the record, the internal inconsistency is still real (`prob_up` below 0.50 on every row while `expected_return` is large and positive on 8 of 10; the SELL→HOLD flip on 07-22→07-23 with `prob_up` essentially unchanged) — it's just evidence about a dead system, cited here purely as historical context for why rule #11 exists, not as anything resembling a live signal. Nothing here informed §3, and nothing here should inform a real decision.
...
**What was shelved:** Model A (ML-based `prob_up` / `expected_return` forecasting) was quarantined by standing policy on 2026-07-11, then had its runtime physically removed from the codebase on 2026-08-13 (PR #100, §4) — it does not, and now structurally cannot, feed thesis construction, allocation, or this evaluation.
