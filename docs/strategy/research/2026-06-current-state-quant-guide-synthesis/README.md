...
**A — P0 signal correctness.** The read-only verification is **done in this pack (both CONFIRMED)**; the next concrete step is the **scoped, approved P0 fix** (expected_return scale/threshold re-derivation + adj_close+retrain plan), with a **brief honesty caveat shipped as a same-change safety rider**. Justification in `06`. Everything else (price-metadata design `B`, baseline/rank-IC research `D`) runs in **parallel as read-only**, but must not precede the correctness fix in *implementation* order.
