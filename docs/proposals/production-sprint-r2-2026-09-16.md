...
| **S3** paper book | `jobs/snapshot_paper_book.py` writes `paper_book_snapshots` daily from `ASXOS_PAPER_CAPITAL_AUD` (arbi-declared, labelled paper) + holdings; re-point `has_enough_paper_weeks` (`portfolio/paper_trade.py:295`). After M1. | Amber | job, `paper_trade.py`, `daily-brief.yml` | M1, R1 | ≥ 7 consecutive `as_of` rows; gate reachable in a unit test | `captures:` snapshots | C-23 |
...
DECISION: the paper book is arbi-declared (paper is arbi's, §2.2).
TAKING:   ASXOS_PAPER_CAPITAL_AUD in the workflow, every render labelled "paper, arbi-declared";
